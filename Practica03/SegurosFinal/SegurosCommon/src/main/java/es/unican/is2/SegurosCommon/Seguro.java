...
public class Seguro {
	...
	public double precio() {
		...
		double multiplicadorPotencia = 1;
        if (potencia >= 90 && potencia <= 110) {
            multiplicadorPotencia = 1.05;
        } else if (potencia > 110) {
            multiplicadorPotencia = 1.2;
        }

        double precioTotal = precioBase * multiplicadorPotencia;
        
        if (hoy.isBefore(fechaInicio.plusYears(1)) || hoy.isEqual(fechaInicio)) {
